...
function EmailPreferenceInner({ action }: { action: EmailPrefAction }) {
  ...
  // A missing token usually means the link was already used (some email
  // clients and link scanners strip or already consumed the query string),
  // not that anything is actually broken, so it gets its own, calmer copy
  // rather than the "This link didn't work" error state.
  const [state, setState] = useState<"loading" | "done" | "error" | "no-token">(token ? "loading" : "no-token");
      ...
      <div role="status" aria-live="polite">
        ...
        <p className="mt-3 text-[14.5px] leading-[1.6] text-ink-muted">
          {state === "done"
            ? copy.doneBody
            : state === "no-token"
            ? "If you already used this link, you're all set. Otherwise, open the link from your email again."
            : state === "error"
