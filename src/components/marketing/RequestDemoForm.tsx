...
import { requestDemo } from "@/lib/demo-api";
...
// Preset slots are far more reliable on mobile than a native datetime picker,
// and the exact time gets nailed down on WhatsApp anyway.
const TIME_SLOTS = [
  "As soon as possible",
  "Weekday morning (9am to 12pm)",
  "Weekday afternoon (12pm to 4pm)",
  "Weekday evening (4pm to 6pm)",
  "Weekend",
];
...
export function RequestDemoForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    // Capture the lead server-side first (stores it, alerts sales, and adds the
    // email to the nurture list). Fire-and-forget: requestDemo never throws, and
    // we must NOT await before window.open or the popup blocker will kill the
    // WhatsApp tab (window.open only works synchronously inside the gesture).
    void requestDemo({ fullName, email, phone, role, preferredAt, message });
