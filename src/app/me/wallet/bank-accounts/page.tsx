...
export default function TenantBankAccountsPage() {
  ...
  const setPrimary = useMutation({
    mutationFn: (id: string) => tenantApi.setPrimaryBankAccount(id),
    onSuccess: () => {
  ...
  const remove = useMutation({
    mutationFn: (id: string) => tenantApi.deleteBankAccount(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["bank-accounts"] });
  ...
  return (
    <>
      <TenantTopbar
        title="Bank accounts"
        subtitle="Where your referral earnings are paid. Withdrawals need an account in your own name."
        actions={
...
function AddBankAccountForm({
  ...
  const add = useMutation({
    mutationFn: () => {
      ...
      return tenantApi.addBankAccount({
        accountNumber,
        bankCode,
        bankName: bank?.name ?? "",
