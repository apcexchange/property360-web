...
- Any native build config beyond registering the `expo-notifications` config plugin in `app.json`'s `plugins` array (needed so `expo prebuild` sets the iOS `aps-environment` entitlement and Android notification metadata; the plugin does not run automatically just from the package being installed).
...
**Chat integration:** `ChatService.sendMessage` and `BuildingChatService.sendMessage`, invoked from the socket handlers in `socketServer.ts`, currently only emit to the conversation/building room. After the message is persisted, add a call to `NotificationService.createNotification(recipientId, senderName, messageText, 'chat', { conversationId }, { respectPreferences: true })` for 1:1 chat (and the equivalent per-recipient fan-out for building chat). To avoid double-buzzing someone actively viewing the conversation, skip creating the `Notification` document entirely (push and in-app notification-center entry alike — the room event is already what showed them the message) when the recipient already has a live socket joined to that `conversation:${id}` / `building:${propertyId}` room, checked via a local in-memory lookup (`onlineUsers` map intersected with `io.sockets.adapter.rooms`, not Socket.IO's `fetchSockets()` — this codebase sets custom properties directly on the socket instance rather than on `socket.data`, and `fetchSockets()` only serializes `.data`, so it would never see them and the presence check would silently always report "absent"). This only works because Socket.IO here is explicitly single-instance and not Redis-shared; the room map is authoritative in-process state.
